...
#define RLED 3  //RED_LED
#define GLED 5  //GREEN_LED
#define BLED 6 //BLUE_LED
...
bool Checksum(uint8_t _r, uint8_t _g, uint8_t _b, uint16_t _key)
{
	...
	//Serial.println(_key);
	return ((uint8_t)buffer == (uint8_t)_key) ? true : false;
}
...
{
	analogWrite(RLED, 255 - _r);
	analogWrite(GLED, 255 - _g);
	analogWrite(BLED, 255 - _b);
	//Serial.print("R: ");
	//Serial.println(_r);
	//Serial.print("G: ");
	//Serial.println(_g);
